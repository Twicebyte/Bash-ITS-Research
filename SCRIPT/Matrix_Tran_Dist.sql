SELECT y.GROUP_NUMBER,y.ROUTE_NAME,y.ROUTE_NUMBER,BNAME,ENAME,AMOUNT, DIST FROM (
select ROW_NUMBER() OVER(PARTITION BY RGN,EGN,BGN ORDER BY abs(a.LATITUDE-b.LATITUDE)+abs(a.LONGITUDE-b.LONGITUDE) asc ) rang,RGN,a.STOP_NAME BNAME,b.STOP_NAME ENAME,AMOUNT, DIST FROM (
...
join ULTIMATE_STOPS a on (a.GROUP_NUMBER = x.BGN)
join ULTIMATE_STOPS b on (b.GROUP_NUMBER = x.EGN)
) x 
