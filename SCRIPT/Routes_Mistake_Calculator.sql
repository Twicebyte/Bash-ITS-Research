SELECT RGN, ROUTE_NAME,ROUTE_NUMBER,AMOUNT,AIM,DIFF --into somecrap2 
FROM (
SELECT ROW_NUMBER() OVER(PARTITION BY RGN ORDER BY LEN(ROUTE_NAME) asc) rang, RGN, ROUTE_NAME,ROUTE_NUMBER,AMOUNT,AIM,DIFF FROM (
...
join ULTIMATE_ROUTES y on (x.RGN=y.GROUP_NUMBER and SOURCE=1)
)x where rang=1
order by DIFF DESC