SELECT RGN,RNAME,RNUMBER,isnull([1],CONCAT('AMOUNT: ',STR(0),'; AVGDIST: ',STR(0))) as WITH_UFA,isnull([0],CONCAT('AMOUNT: ',STR(0),'; AVGDIST: ',STR(0))) as NO_UFA FROM (
Select RGN,RNAME,RNUMBER,UFA_BOUND,CONCAT('AMOUNT: ',STR(AMOUNT),'; AVGDIST: ',STR(Dist/Amount)) VAL FROM (
SELECT y.RGN,y.RNAME,y.RNUMBER,UFA_BOUND,SUM(AMOUNT) Amount, SUM(AMOUNT*Dist) DIST FROM (
SELECT RGN,BGN,EGN,UFA_BOUND,SUM(AMOUNT) AMOUNT, avg(abs(ac.DISTANCE - bc.DISTANCE)) DIST FROM (
select 
2 SOURCE, 
...
END UFA_BOUND,
AMOUNT from ULTIMATE_DS_TRANSACT a 
...
CASE
	...
	WHEN (c.CITY_NUMBER not like N'Уфа%' and d.CITY_NUMBER not like N'Уфа%') THEN 0
END UFA_BOUND,
AMOUNT from ULTIMATE_RISE_TRANSACT a 
join ULTIMATE_ROUTES b on (a.ROUTE_ID=b.ID and b.SOURCE=0)
...
group by y.RGN,y.RNAME,y.RNUMBER,UFA_BOUND
) x
