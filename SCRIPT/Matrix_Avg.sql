SELECT RNAME,RNUMBER,M,ROUND(100*M/(M+N+E+G),2) MP,N,ROUND(100*N/(M+N+E+G),2) NP,E,ROUND(100*E/(M+N+E+G),2) EP,G,ROUND(100*G/(M+N+E+G),2) GP, M+N+E+G A
FROM (
SELECT RNAME,RNUMBER,
SUM(M) M,SUM(N) N,SUM(E) E,SUM(G) G
FROM (
SELECT RNAME,RNUMBER,BSTOP,BLA,BLO,ESTOP,ELA,ELO,
ACTIVEDAYS,M,N,E,G
FROM (
...
--ORDER BY (M+N+E+G) DESC
) x GROUP BY RNAME,RNUMBER
) x
ORDER BY (M+N+E+G) DESC