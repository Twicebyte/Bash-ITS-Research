...
END DAYPART,DATEPART(DD,a.DDATE) DDAY,DATEPART(MM,a.DDATE) DMON,DATEPART(DW,a.DDATE) DDOW, b.ID RGN,c.GROUP_NUMBER BGN,d.GROUP_NUMBER EGN,AMOUNT from ULTIMATE_DS_TRANSACT a join (SELECT * FROM ULTIMATE_ROUTES WHERE GROUP_NUMBER=21) b 
...
END DAYPART,DDAY,DMON,DDOW, b.ID RGN,BGN,EGN,AMOUNT from ULTIMATE_RISE_TRANSACT a join (SELECT * FROM ULTIMATE_ROUTES WHERE GROUP_NUMBER=21) b 
on (a.ROUTE_ID=b.ID)
) x
...
select distinct
DATEPART(DD,a.DDATE) DDAY,DATEPART(MM,a.DDATE) DMON,DATEPART(DW,a.DDATE) DDOW,b.ID RGN from ULTIMATE_DS_TRANSACT a join (SELECT * FROM ULTIMATE_ROUTES WHERE GROUP_NUMBER=21) b 
on (a.IdDataSource=b.SOURCE_REGION and a.IdRoute=b.SOURCE_CODE and b.SOURCE=2)
UNION
select distinct DDAY,DMON,DDOW, b.ID RGN from ULTIMATE_RISE_TRANSACT a join (SELECT * FROM ULTIMATE_ROUTES WHERE GROUP_NUMBER=21) b 
on (a.ROUTE_ID=b.ID)
) x 
--[[[FILTER BEGIN]]]
--where 
...
) x join (SELECT * FROM ULTIMATE_ROUTES WHERE GROUP_NUMBER=21) y on (x.RGN=y.ID)
